#include "lemlib/motions/swings.hpp"
 ...
 */
DifferentialSwing::DifferentialSwing(PID& angularPID, float target, int timeout, DriveSide lockedSide,
                                     SwingToParams& params)
 ...
 * initial competition state.
 */
DifferentialSwing::DifferentialSwing(PID& angularPID, Pose target, int timeout, DriveSide lockedSide,
                                     SwingToParams& params)
    : angularPID(angularPID),
      targetPose(target),
      params(params),
 ...
 */
DifferentialChassisSpeeds DifferentialSwing::calculate(Pose pose) {
    switch (state) {
        case 1: return {0, 0};
