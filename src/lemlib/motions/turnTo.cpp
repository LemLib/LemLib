 ...
 * @param direction what direction to force motion, optionally. Nullopt by
 * default
 *
 * @return Angle the angular error
 */
static Angle
calculateError(std::variant<Angle, V2Position> target,
               const Pose& pose,
               std::optional<AngularDirection> direction = std::nullopt) {
    // if the target is an angle
    if (std::holds_alternative<Angle>(target)) {
        return angleError(std::get<Angle>(target), pose.orientation, direction);
    } else { // otherwise, the target is a pose
        return pose.angleTo(std::get<V2Position>(target));
    }
}

void turnTo(std::variant<Angle, V2Position> target,
            Time timeout,
            TurnToParams params,
            TurnToSettings settings) {
    // print debug info
    if (std::holds_alternative<Angle>(target)) {
        logHelper.info("Turning to {:.2f}", std::get<Angle>(target));
    } else {
        logHelper.info("Turning to face point {:.2f}",
                       std::get<V2Position>(target));
    }

    // figure out which way to limit acceleration
    const SlewDirection slewDirection = [&] {
        if (params.direction == AngularDirection::CCW_COUNTERCLOCKWISE) {
            return SlewDirection::INCREASING;
        }
        if (params.direction == AngularDirection::CW_CLOCKWISE) {
            return SlewDirection::DECREASING;
        }
        const Pose pose = settings.poseGetter();
        const Angle error = calculateError(target, pose);
        return error > 0_stDeg ? SlewDirection::INCREASING :
                                 SlewDirection::DECREASING;
    }();
    ...
    if (params.lockedSide) {
        if (*params.lockedSide == TurnToParams::LockedSide::LEFT) {
            settings.leftMotors.setBrakeMode(BrakeMode::BRAKE);
        } else {
            settings.rightMotors.setBrakeMode(BrakeMode::BRAKE);
        }
    }

    lemlib::MotionCancelHelper helper(10_msec); // cancel helper
    // loop until the motion has been cancelled, the timer is done, or an exit
    // condition has been met
    while (helper.wait() && !timer.isDone() &&
           !settings.exitConditions.update(deltaTheta)) {
        ...
        deltaTheta = [&] {
            ...
            settling = prevRawDeltaTheta != std::nullopt &&
                       (sgn(raw) != sgn(prevRawDeltaTheta.value()));
            prevRawDeltaTheta = raw;
            const Angle error =
              calculateError(target,
                             pose,
                             settling ? std::nullopt : params.direction);
        ...
        if (params.minSpeed != 0 && abs(deltaTheta) < params.earlyExitRange) {
            break;
        }
        if (params.minSpeed != 0 &&
            sgn(deltaTheta) != sgn(prevDeltaTheta.value())) {
            break;
        }
        ...
        // print debug info
        logHelper.debug(
          "Turning with {:.4f} power, error: {:.2f} stDeg, dt: {:.4f}",
          motorPower,
          to_stDeg(deltaTheta),
          helper.getDelta());
