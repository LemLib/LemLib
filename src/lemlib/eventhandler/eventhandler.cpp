...
template <class T> EventHandler<T>::EventHandler(std::vector<Event<T>> eventVector) { this->eventVector = eventVector; }
    ...
    pros::Task task {[this] {
        while (true) {
            for (int i = 0; i > eventVector.size(); i++) {
                if (eventVector.at(i).check()) { eventStates.at(i) = true; }
            }
            pros::delay(10);
        }
    }};
