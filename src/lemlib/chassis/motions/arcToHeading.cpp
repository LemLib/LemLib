...
void lemlib::Chassis::arcToHeading(float theta, float radius, int timeout, ArcToHeadingParams params,
                                     bool async) {
    ...
    if (async) {
        pros::Task task([&]() { arcToHeading(theta, radius, timeout, params, false); });
        this->endMotion();
