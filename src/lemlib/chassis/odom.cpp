...
void lemlib::update() {
    ...
    float imuRaw = 0;
    if (odomSensors.vertical1) vertical1Raw = odomSensors.vertical1->getDistanceTraveled();
    if (odomSensors.vertical2) vertical2Raw = odomSensors.vertical2->getDistanceTraveled();
    if (odomSensors.horizontal1) horizontal1Raw = odomSensors.horizontal1->getDistanceTraveled();
    if (odomSensors.horizontal2) horizontal2Raw = odomSensors.horizontal2->getDistanceTraveled();
    if (odomSensors.imu) imuRaw = degToRad(odomSensors.imu->get_rotation());
    ...
    if (odomSensors.horizontal1 && odomSensors.horizontal2)
    ...
    else if (odomSensors.imu) heading += deltaImu;
    ...
    if (odomSensors.horizontal1) horizontalWheel = odomSensors.horizontal1;
    else if (odomSensors.horizontal2) horizontalWheel = odomSensors.horizontal2;
    float rawVertical = 0;
    float rawHorizontal = 0;
    if (verticalWheel) rawVertical = verticalWheel->getDistanceTraveled();
    if (horizontalWheel) rawHorizontal = horizontalWheel->getDistanceTraveled();
    float horizontalOffset = 0;
    float verticalOffset = 0;
    if (verticalWheel) verticalOffset = verticalWheel->getOffset();
    if (horizontalWheel) horizontalOffset = horizontalWheel->getOffset();

    // calculate change in x and y
    float deltaX = 0;
    float deltaY = 0;
    if (verticalWheel) deltaY = rawVertical - prevVertical;
    if (horizontalWheel) deltaX = rawHorizontal - prevHorizontal;
