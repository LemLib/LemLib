...
void lemlib::Chassis::moveToPose(MoveToPoseTarget targetPose, int timeout,
  ...
  MovementType mType;
  if (!isnan(targetPose.x) && !isnan(targetPose.y) &&
      !isnan(targetPose.theta)) {
  ...
  Pose target =
      Pose(targetPose.x, targetPose.y, M_PI_2 - degToRad(targetPose.theta));
  switch (mType) {
  case RelativeWithoutAngle:
    target =
        Pose(pose_t.x + targetPose.dist * std::cos(pose_t.theta),
             pose_t.y + targetPose.dist * std::sin(pose_t.theta), pose_t.theta);
    break;
  case RelativeWithAngle:
    target =
        Pose(pose_t.x + targetPose.dist * std::cos(degToRad(targetPose.theta)),
             pose_t.y + targetPose.dist * std::sin(degToRad(targetPose.theta)),
             targetPose.theta);
    break;
