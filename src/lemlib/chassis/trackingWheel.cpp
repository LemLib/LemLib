...
void lemlib::TrackingWheel::reset() {
    ...
    if (this->motors) this->motors->tare_position();
}
...
float lemlib::TrackingWheel::getDistanceTraveled() {
    if (this->encoder) return float(this->encoder->get_value()) * this->diameter * M_PI / 360;
    else if (this->rotation) return float(this->rotation->get_position()) * this->diameter * M_PI / 36000;
    else if (this->motors) {
