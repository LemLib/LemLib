...
template <typename Char> void vformat_to(buffer<Char>& buf, const text_style& ts, basic_string_view<Char> format_str,
...
template <typename T, typename Char> struct formatter<detail::styled_arg<T>, Char> : formatter<T, Char> {
        template <typename FormatContext> auto format(const detail::styled_arg<T>& arg, FormatContext& ctx) const
            -> decltype(ctx.out()) {
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename T> FMT_CONSTEXPR auto styled(const T& value, text_style ts)
