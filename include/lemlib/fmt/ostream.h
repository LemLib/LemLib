...
inline bool write_ostream_unicode(std::ostream& os, fmt::string_view data) {
    ...
    auto* rdbuf = os.rdbuf();
    FILE* c_file;
    if (auto* sfbuf = dynamic_cast<__gnu_cxx::stdio_sync_filebuf<char>*>(rdbuf)) c_file = sfbuf->file();
    else if (auto* fbuf = dynamic_cast<__gnu_cxx::stdio_filebuf<char>*>(rdbuf)) c_file = fbuf->file();
    else return false;
...
FMT_EXPORT template <typename Char> void vprint(std::basic_ostream<Char>& os,
...
FMT_EXPORT template <typename... T> void print(std::ostream& os, format_string<T...> fmt, T&&... args) {
    ...
    if (detail::is_utf8()) vprint(os, fmt, vargs);
    else detail::vprint_directly(os, fmt, vargs);
}
