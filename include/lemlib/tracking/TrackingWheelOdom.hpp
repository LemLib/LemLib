...
class TrackingWheel {
  public:
     ...
     *
     * @param port the port of the rotation sensor. To reverse, make the port
     * negative
     * @param diameter the diameter of the wheel
     * @param offset how far the tracking wheel is from the turning center
     * @param ratio the gear ratio, driven gear / driving gear. Defaults to 1
     ...
     * @b Example:
     * @code {.cpp}
     * // create a tracking wheel with a rotation sensor on port 1, which is
     * reversed,
     * // has a diameter of 2.75 inches, and is offset 2 inches to the right
     * lemlib::TrackingWheel trackingWheel(-1, 2.75_in, 2_in);
     * @endcode
     */
    TrackingWheel(ReversibleSmartPort port,
                  Length diameter,
                  Length offset,
                  Number ratio = 1);
    /**
     * @brief Construct a new Tracking Wheel object using an Optical Shaft
     * Encoder
     *
     ...
     * @b Example:
     * @code {.cpp}
     * // create a tracking wheel with an optical shaft encoder on ports 'A' and
     * 'B',
     * // which is not reversed, has a diameter of 2 inches, and is offset 4
     * inches
     * // to the right, and has a gear ratio of 2:1
     * lemlib::TrackingWheel trackingWheel('A', 'B', false, 2_in, 4_in, 2);
     * @endcode
     */
    TrackingWheel(ADIPort topPort,
                  ADIPort bottomPort,
                  bool reversed,
                  Length diameter,
                  Length offset,
     ...
     * @b Example:
     * @code {.cpp}
     * // create a tracking wheel with an optical shaft encoder on ports 'E' and
     * 'F',
     * // on an ADI expander on port 3, which is reversed, has a diameter of 2
     * inches,
     * // and is offset 0 inches
     * lemlib::TrackingWheel trackingWheel(3, 'E', 'F', true, 2_in, 0_in);
     * @endcode
     */
    TrackingWheel(SmartPort expanderPort,
                  ADIPort topPort,
                  ADIPort bottomPort,
                  bool reversed,
                  Length diameter,
                  ...
                  Number ratio = 1);
    /**
     * @brief Get the distance traveled by the tracking wheel since this
     * function was last called. This function is not thread safe.
     *
     * It is recommended to set the angle of the encoder to 0 before starting to
     * use this function.
     *
     * Since the internal encoder object is abstract, it's not known what values
     * errno may be set to in case of a failure.
     *
     * @return INFINITY an error has occurred, possibly setting errno
     * @return Length the distance the tracking wheel has traveled since the
     ...
     *   lemlib::V5RotationSensor encoder(3, true);
     *   lemlib::TrackingWheel trackingWheel(&encoder, 2.75_in, -3_in);
     *   std::cout << to_in(trackingWheel.getOffset()) << std::endl; // outputs
     * -3
     * }
     * @endcode
     */
     ...
     *   lemlib::TrackingWheel trackingWheel(&encoder, 2.75_in, -3_in);
     *   trackingWheel.reset();
     *   std::cout << to_in(encoder.getDistanceTraveled()) << std::endl; //
     * outputs 0
     * }
     * @endcode
     */
     ...
     *
     * This function deallocates the encoder pointer unless the pointer was
...
class TrackingWheelOdometry {
  public:
     ...
     * lemlib::TrackingWheel horizontalWheel1(&horizontalEncoder1, 3.25_in,
     * -2_in); lemlib::TrackingWheel
     * horizontalWheel2(&horizontalEncoder2, 2.75_in, -4_in);
     *
     * // create a TrackingWheelOdometry instance
     * lemlib::TrackingWheelOdometry odom({}, {verticalWheel},
     * {horizontalWheel1, horizontalWheel2});
     * @endcode
     */
    TrackingWheelOdometry(std::vector<IMU*> imus,
     ...
     *
     * @param period how long to wait before updating the task again. Defaults
     * to 10 ms
     *
     * @b Example:
     * @code {.cpp}
    ...
    /**
     * @brief Destroy the Tracking Wheel Odometry object. Stops the tracking
     * task
     *
