...
class InputReader {
    private:
        ...
        static constexpr uint16_t inputMapLength = 3000; // Number of ticks to save inputs for. After the tick limit is
                                                         // reached, the inputs will no longer be saved.
    ...
    public:
         ...
         */
        InputReader(LEMController* controllerArg, std::vector<LEMButtonMapping> buttonsToFunctions,
                    std::vector<int (*)(int)> joystickFunctions);
         ...
         * @param buttonsToFunctions The SAME EXACT BUTTONS TO FUNCTIONS VECTOR FROM THE CONTROLLER.
         */
        InputReader(LEMController* controllerArg, Drivetrain* drivetrain,
                    std::vector<LEMButtonMapping> buttonsToFunctions);
    ...
    protected:
        static int16_t
            readInputsBuffer[inputSourceNum * inputMapLength]; // The buffer between the read inputs and a save file.
