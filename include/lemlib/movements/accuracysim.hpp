...
class AccuracySIM {
    private:
        int amountOfSkillsSimsDone; // The amount of skills simulations that have been done. Used for saving the data to
                                    // the file.
        ...
        std::vector<float>
            maxDistances; // Vector of the maximum distance that the driver was from the current targeted circle

        float lastTimeEnteredCircle; // The time that the driver last entered a circle, used for calculating the time it
                                     // took to go through the circle. Recorded in milliseconds.

        Rectangle botDimensions; // Rectangle with the dimensions of the bot. Be as accurate as possible, as this
                                 // will be used for the performance evaluation. Only measure relevant things: an axle
                                 // sticking out of the bot or a random piece of metal that doesn't affect the bot's
                                 // performance doesn't need to be included. However, if you have a mechanism that
                                 // extends out of the bot, that should be included.
        ...
        /**
         * @brief Records the distance from the current targeted circle over time, and sets it to the vector of max
         * distances.
         *
         */
    ...
    public:
         ...
         * @param autonSelectorArg Pointer to the auton selector that can be either inputted or not. If not inputted,
         * the auton will not be run and by default driver control will run.
         * @param inputReaderArg Pointer to the input reader that can be either inputted or not. If not inputted, the
         */
        AccuracySIM(LEMController* controllerArg, AutonSelector* autonSelectorArg = nullptr,
                    InputReader* inputReaderArg = nullptr);
