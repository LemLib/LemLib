...
typedef struct {
        ...
        struct {
                ...
                lv_coord_t valid_x; /*Used when stepping up/down in text area when stepping to a shorter line. (Handled
                                       by the library)*/
...
/**
 * Enable the scroll propagation feature. If enabled then the Text area will move its parent if there is no more space
 * to scroll.
 * @param ta pointer to a Text area
