...
enum {
    LV_RES_INV = 0, /*Typically indicates that the object is deleted (become invalid) in the action function or an
...
enum {
    ...
    LV_PROTECT_PRESS_LOST =
        0x10, /*If the `indev` was pressing this object but swiped out while pressing do not search other object.*/
...
typedef struct {
        const char*
            type[LV_MAX_ANCESTOR_NUM]; /*[0]: the actual type, [1]: ancestor, [2] #1's ancestor ... [x]: "lv_obj" */
} lv_obj_type_t;
