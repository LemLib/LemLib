...
template <typename Char, size_t N, fmt::detail_exported::fixed_string<Char, N> Str> struct udl_compiled_string
    : compiled_string {
...
constexpr const auto& get([[maybe_unused]] const T& first, [[maybe_unused]] const Args&... rest) {
    ...
    else return detail::get<N - 1>(rest...);
}
...
template <typename Char> struct runtime_named_field {
        ...
        template <typename OutputIt, typename T> constexpr static bool
...
template <typename Args, size_t POS, int ID, typename T, typename S> constexpr auto parse_tail(T head, S format_str) {
    if constexpr (POS != basic_string_view<typename S::char_type>(format_str).size()) {
        ...
        if constexpr (std::is_same<remove_cvref_t<decltype(tail)>, unknown_format>()) return tail;
        else return make_concat(head, tail);
    } else {
        return head;
    }
}
